...
class CT(EDCT):
    ...
    def forward(self, batch, detach_treatment=False):
        ...
        if self.training and self.hparams.model.multi.augment_with_masked_vitals and self.has_vitals: #훈련 중이고, vital augmentation 옵션이 켜져있고, vital sign이 존재하는 경우
            # Augmenting original batch with vitals-masked copy
            assert fixed_split is None  # Only for training data에만 강제함
            ...
            for i, seq_len in enumerate(batch['active_entries'].sum(1).int()):
                ...
                # torch.randint로 랜덤한 시점을 고르고, 그 시점까지만 vitals가 있는 것처럼 만듭니다.

            for (k, v) in batch.items():
                batch[k] = torch.cat((v, v), dim=0) # 원본 + 마스킹본을 동시에 학습하기 위해 합침(batch 안에 있는 모든 키 (prev_outputs, vitals, current_treatments 등)에 대해)
        ...
        treatment_pred = self.br_treatment_outcome_head.build_treatment(br, detach_treatment) # treatment 예측 gradient reverse가 추가되어있음
        outcome_pred = self.br_treatment_outcome_head.build_outcome(br, curr_treatments) # outcome 예측
    ...
    def build_br(self, prev_treatments, vitals, prev_outputs, static_features, active_entries, fixed_split):
        ...
        for block in self.transformer_blocks:
            ...
            if self.has_vitals:
                ...
                x_t, x_o = block((x_t, x_o), x_s, active_entries_treat_outcomes) # seq_hidden_units
            ...
            if fixed_split is not None:  # Test seq data
                ...
                for i in range(len(active_entries)):
                    ...
                    x[i, :int(fixed_split[i])] = \
                        (x_o[i, :int(fixed_split[i])] + x_t[i, :int(fixed_split[i])] + x_v[i, :int(fixed_split[i])]) / 3 # 0 ~ fixed split 까지는 treat + outcome + vital의 평균
                    x[i, int(fixed_split[i]):] = (x_o[i, int(fixed_split[i]):] + x_t[i, int(fixed_split[i]):]) / 2 # fixed split ~ 는 treat + outcome 평균 (vital이 관측되지 않은 상황 반영)
            else:  # Train data always has vitals
                x = (x_o + x_t + x_v) / 3

        output = self.output_dropout(x)
        br = self.br_treatment_outcome_head.build_br(output) # Linear 태워서 마무리
        return br # representation
